...
public class T01_api_static {
	...
	// 인스턴스 메소드
	void me1() {
		System.out.println("인스턴스 me1()");
		System.out.println(numIn);
	 // 인스턴스 메소드를 호출한다는 것 은 이미 객체 생성되어 있다
	 // 인스턴스 변수,메소드, 클래스 변수, 모두 호출 가능	
	...
	// 인스턴스 변수 사용 불가, 인스턴스 변수는(메소드)는 반드시 객체가 생성 되어야만 사용할 수 있기때문	
		System.out.println("정적메소드 sMe3()");
		System.out.println(numSt);
	//	System.out.println(numIn); 오류원인 : 정적메소드에서는 인스턴스 변수 사용불가능
	}
