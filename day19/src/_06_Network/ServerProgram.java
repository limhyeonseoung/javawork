...
public class ServerProgram {
	/*
	 ...
	 - 웹에소는 통신방식 : http 프로토콜 통신(https)

	 * 자바만을 가지고 서버와 클라이언트간에 간단한 통신하기
	 이때 데이터를 입출력 하고자 한다면 서버와 클라이언트간에 스트림(연결통로)이 필요

	 * 소켓 : 프로세스간의 통신을 담당. 스트림을 연결하기 위한 문.
	 * 소켓 프로그래밍 
	   - TCP : 데이터를 정확하고 안정적으로 전달 가능 (신뢰성) 느림.  ex) 이메일, http
	   - UDP : 신뢰성이 없는 데이터 전달.빠름 ex) 실시간 동영상 서비스
	 */
...
/*
			...
			*/
			...
			// 데이터 출력시 print(), println() 메서드를 가지고 있는 보조스트림
